...
`worklog` ships one skill plus five commands. `/worklog:log` captures work activity into a rolling Obsidian `Worklog.md`; `/worklog:weekly-report` and `/worklog:perf-review` draft a weekly status report or a performance-review narrative from it. Helpers (`bin/worklog.py`) run directly on `python3`, no venv. The vault path and report templates are machine-local config (never in this repo), and reports are drafts written into the vault, never sent. Where `jira`/`glab` are available, the reports can augment your hand-logged notes with a **factual pull** — tickets resolved and MRs merged in the range. `/worklog:metric` records numeric KPIs to a separate `Metrics.md` (upserted per day, accepting several `name=value` readings in one call e.g. `work-hours=8 focus-hours=4.5`; `work-hours` is a recommended one to track), and `/worklog:metrics` drafts a trend report from it; the `help` event type feeds the report's derived help-count KPI. Both the weekly report and the perf-review draft now end with a **Metrics (curate before sharing)** section pulled from `Metrics.md` (total + daily average per metric, omitted for metrics with no readings in range) — trim it before using the draft, since nothing is auto-sent.
